...
public final class Network {
    ...
    public func request<T: Decodable>(_ request: URLRequest) -> AnyPublisher<T, Error> {
    ...
    public func requestImage(_ request: URLRequest) -> AnyPublisher<URL, Error> {
        session.downloadTaskPublisher(for: request)
            .map(\.url)
            .mapError(NetworkError.imageDownload)
            .share()
            .eraseToAnyPublisher()
    }
